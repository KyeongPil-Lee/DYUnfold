...
void produce_respM() {
  ...
  const Int_t nMllBin = 36;
  Double_t arr_mllBinEdge[nMllBin+1] = {
  ...
  TH1::SetDefaultSumw2();
  TH1D* h_mll_gen = new TH1D("h_mll_gen", "h_mll_gen", nMllBin, arr_mllBinEdge); // -- gen-level invariant mass
  TH1D* h_mll_rec = new TH1D("h_mll_rec", "h_mll_rec", nMllBin, arr_mllBinEdge); // -- rec-level invariant mass
  TH2D* h_respM = new TH2D("h_respM", "h_respM", nMllBin, arr_mllBinEdge, nMllBin, arr_mllBinEdge); // -- response matrix
  ...
  for (Int_t i_ev = 0; i_ev < nEvent_tot; ++i_ev) {
    ...
    for (Int_t i_lep = 0; i_lep < GenDressedLepton_pt.GetSize(); ++i_lep) {
      if (GenDressedLepton_pdgId[i_lep] == 13) {
    ...
    for (Int_t i_lep = 0; i_lep < Muon_pt.GetSize(); ++i_lep) {
      if (Muon_tightId[i_lep] && Muon_pfRelIso04_all[i_lep] < 0.15 && 
          Muon_pt[i_lep] > 15.0 && Muon_eta[i_lep] < 2.4) {
    ...
    if( hasDY_gen && hasDY_rec ) { // -- if both gen- and rec-level pairs are found
      ...
      if( hasDY_gen )  { // -- if only gen-level pair is found
        h_respM->Fill(-1.0,    mll_gen, *genWeight);
      }
      if( hasDY_rec )  { // -- if only rec-level pair is found
        h_respM->Fill(mll_rec, -1.0, eventWeight);
